...
import { updateCustomerImage } from "../../redux/adminReducer/adminThunk";
...
const AdCustomer = () => {
  ...
  const carouselArr = useSelector((state) => state.userReducer.customer);
  ...
  const handleUpdateClick = async (index) => {
    ...
    if (selectedFile) {
      ...
      dispatch(updateCustomerImage({ formData, img_id: imgId }))
        .then(() => {
    ...
    <div className="flex-1 p-6 bg-gray-50">
      <div className="bg-white shadow rounded-lg p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold">Customer Image</h2>
        </div>
        <div className="py-2">Select a file to update the customer image</div>
      </div>
          ...
          <div key={index} className="w-1/4 px-4 mb-6">
            <div className="text-center mx-auto flex justify-center">
              <img
                ...
                alt=""
                className="w-fit h-32 object-cover mb-2" // Ensure fixed height
              />
            </div>
